		...
		if (ts->black_gesture_support
			...
			if (ts->gesture_enable == 1 || ts->fp_enable == 1) {
				...
				if (ts->fingerprint_underscreen_support)
					ts->ts_ops->enable_fingerprint(ts->
								       chip_data,
								       ! !ts->
								       fp_enable);
				if ((ts->gesture_enable != 1)
				    && ts->ts_ops->enable_gesture_mask)
					ts->ts_ops->enable_gesture_mask(ts->
									chip_data,
									0);
			} else {
				ts->ts_ops->mode_switch(ts->chip_data,
...
static void tp_gesture_handle(struct touchpanel_data *ts)
{
	...
	TPD_INFO("detect %s gesture\n",
		 ...
		 KEY_GESTURE_W ? "(W)" : gesture_info_temp.gesture_type ==
		 KEY_GESTURE_FP_DOWN ? "(fingerprintdown)" : gesture_info_temp.
		 gesture_type == KEY_GESTURE_FP_UP ? "(fingerprintup)" : "unknown");
	...
	if (gesture_info_temp.gesture_type != KEY_GESTURE_UNKNOWN
	    && gesture_info_temp.gesture_type != KEY_GESTURE_FP_DOWN
	    && gesture_info_temp.gesture_type != KEY_GESTURE_FP_UP) {
		...
		input_sync(ts->input_dev);
	} else if (gesture_info_temp.gesture_type == KEY_GESTURE_FP_DOWN) {
		ts->fp_info.touch_state = 1;
		opticalfp_irq_handler(&ts->fp_info);
	} else if (gesture_info_temp.gesture_type == KEY_GESTURE_FP_UP) {
		ts->fp_info.touch_state = 0;
		opticalfp_irq_handler(&ts->fp_info);
	}
}
				      ...
				      struct point_info *points)
{
	...
	int left_edge_cnt = 0, right_edge_cnt = 0, left_center_cnt =
	    0, right_center_cnt = 0;
		...
		for (i = 0; i < ts->max_num; i++) {
			...
			if (((obj_attention & TOUCH_BIT_CHECK) >> i) & 0x01
				...
				if (ts->edge_limit_support) {
					if (corner_point_process
					    (ts, corner, points, i)
					    || (!ts->drlimit_remove_support
						&& edge_point_process(ts,
								      points
								      [i])))
						continue;
				}
...
static void tp_work_func(struct touchpanel_data *ts)
{
	...
	if (ts->ts_ops->u32_trigger_reason) {
		cur_event =
		    ts->ts_ops->u32_trigger_reason(ts->chip_data,
						   (ts->gesture_enable
						    || ts->fp_enable),
						   ts->is_suspended);
	} else {
		cur_event =
		    ts->ts_ops->trigger_reason(ts->chip_data,
					       (ts->gesture_enable
						|| ts->fp_enable),
					       ts->is_suspended);
	}
...
static void tp_fw_update_work(struct work_struct *work)
{
	...
	if (ts->ts_ops->bootup_test && ts->health_monitor_support) {
		ret =
		    request_firmware(&fw, ts->panel_data.test_limit_name,
				     ts->dev);
		if (ret < 0) {
...
static int init_touchpanel_proc(struct touchpanel_data *ts)
{
	...
	if (ts->fw_update_app_support) {
		...
		register_device_proc("tp",
				     ...
				     ts->panel_data.manufacture_info.
				     manufacture);
	}
	...
	prEntry_tmp =
	    proc_create("debug_level", 0644, prEntry_tp,
	...
	prEntry_tmp =
	    proc_create_data("tp_fw_update", 0666, prEntry_tp,
	...
	if (ts->edge_limit_support || ts->fw_edge_limit_support) {
		prEntry_tmp =
		    proc_create_data("oppo_tp_limit_enable", 0664, prEntry_tp,
		...
		prEntry_tmp =
		    proc_create_data("oppo_tp_direction", 0666, prEntry_tp,
		...
		prEntry_tmp =
		    proc_create_data("oppo_tp_limit_whitelist", 0666,
		...
		if (ts->edge_limit_support) {
			prEntry_tmp =
			    proc_create_data("oppo_tp_limit_area", 0664,
					     prEntry_tp, &proc_limit_area_ops,
					     ts);
			if (prEntry_tmp == NULL) {
				ret = -ENOMEM;
				TPD_INFO("%s: Couldn't create proc entry, %d\n",
	...
	prEntry_tmp =
	    proc_create_data("oppo_register_info", 0664, prEntry_tp,
	...
	if (ts->noise_modetest_support) {
		prEntry_tmp =
		    proc_create_data("oppo_tp_noise_modetest", 0664, prEntry_tp,
...
static ssize_t proc_freq_hop_write(struct file *file,
				   const char __user * buffer, size_t count,
				   loff_t * ppos)
{
	int value = 0;
...
{
	int i = 0, j = 0, max_array_x = 0, max_array_y = 0, eli_array_x =
	    0, eli_array_y = 0;
	struct touchpanel_data *ts = s->private;
...
static ssize_t health_monitor_control(struct file *file,
				      const char __user * buf, size_t count,
				      loff_t * lo)
{
	struct touchpanel_data *ts = PDE_DATA(file_inode(file));
	char buffer[4] = { 0 };
	int tmp = 0;
	int *p_hor = NULL, *p_ver = NULL, eli_unit = 0, hor_range =
	    0, ver_range = 0;
...
//    for (i = 0; i < sizeof(coord_arg)/sizeof(coord_arg[0]); i++)  {
	for (i = 0; i < sizeof(coord_arg) / sizeof(coord_arg[0]); i++) {
		...
		if (coord_arg[i][0] == KEY_GESTURE_DOUBLE_SWIPE)
			i++;
...
static int init_debug_info_proc(struct touchpanel_data *ts)
{
	...
	if (ts->health_monitor_support) {
		...
		prEntry_tmp =
		    proc_create_data("health_monitor", 0666, prEntry_debug_info,
	...
	if (ts->freq_hop_simulate_support) {
		...
		prEntry_tmp =
		    proc_create_data("freq_hop_simulate", 0666,
				     prEntry_debug_info, &proc_freq_hop_fops,
				     ts);
		if (prEntry_tmp == NULL) {
			ret = -ENOMEM;
			TPD_INFO("%s: Couldn't create proc entry, %d\n",
	...
	if (ts->health_monitor_support) {
		ts->monitor_data.eli_ver_pos =
		    kzalloc((ts->monitor_data.eli_ver_range /
			    ...
			    (ts->resolution_info.max_y /
			     ts->monitor_data.eli_size) * sizeof(int),
			    GFP_KERNEL);
		ts->monitor_data.eli_hor_pos =
	...
	if (ts->use_resume_notify) {
		...
		if ((0 == timed_out) || (ts->resume_complete.done)) {
			...
			if (!timed_out && ts->ts_ops->resume_timedout_operate) {
				ts->ts_ops->resume_timedout_operate(ts->
								    chip_data);
				ts->suspend_state = TP_SPEEDUP_RESUME_COMPLETE;
				TPD_INFO("%s: end!\n", __func__);
				mutex_unlock(&ts->mutex);
