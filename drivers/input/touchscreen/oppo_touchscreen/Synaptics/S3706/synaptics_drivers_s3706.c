...
static void synaptics_health_zone_init(struct chip_data_s3706 *chip_info)
{
	...
	if (1 == ret) {
		...
		TPD_INFO
		    ("l_tmp = 0x%x h_tmp = 0x%x chip_info->health_zone.loglength_addr = 0x%x \n",
					 ...
					 bool enable)
{
	...
	if ((LANDSCAPE_SCREEN_90 == chip_info->touch_direction) && !enable) {
		//set area parameter
		if ((0xFF == pre_set_1) && (0x44 == pre_set_2)
		    && (0x03 == pre_set_3)) {
		...
		if ((0x24 == pre_set_1) && (0xF5 == pre_set_2)
		    && (0x05 == pre_set_3)) {
			TPD_INFO("%s: do not enable twice dir = %d\n", __func__,
				 chip_info->touch_direction);
			return 0;
					...
					bool enable)
{
	...
	if (enable) {
		ret |=
		    touch_i2c_write_byte(chip_info->client,
					 chip_info->reg_info.
					 F51_CUSTOM_DATA_BASE + 0x29, 0x01);
	} else {
		ret |=
		    touch_i2c_write_byte(chip_info->client,
					 chip_info->reg_info.
					 F51_CUSTOM_DATA_BASE + 0x29, 0x00);
	}
	touch_i2c_write_byte(chip_info->client, 0xff, 0x00);	//set page 0
				      ...
				      struct gesture_info *gesture)
{
	...
	if (gesture->gesture_type != KEY_GESTURE_UNKNOWN) {
		...
		TPD_INFO
		    ("%s, gesture_sign: 0x%x, gesture_type: %d, clockwise: %d, points: (%d, %d)(%d, %d)(%d, %d)(%d, %d)(%d, %d)(%d, %d)\n",
...
static int synaptics_auto_test_rt155_fd(struct seq_file *s,
					struct chip_data_s3706 *chip_info,
					struct syna_testdata *syna_testdata,
					uint8_t * data_buf)
{
	...
	const int node_num =
	    syna_testdata->TX_NUM + syna_testdata->RX_NUM, judge_threshold =
	    300;
	...
	for (j = 0; j < syna_testdata->RX_NUM; j++) {
		...
		if (max_node_data > judge_threshold) {
			TPD_INFO
			    ("Check RT155 for fd abs noise, data[%d] = %d(%d)\n",
...
static int synaptics_auto_test_rt59(struct seq_file *s,
				    struct chip_data_s3706 *chip_info,
				    struct syna_testdata *syna_testdata,
				    uint8_t * data_buf)
{
	...
	int error_count = 0;
	int i = 0, j = 0;
	const int data_cycle = 20, node_num =
	    syna_testdata->TX_NUM + syna_testdata->RX_NUM, judge_threshold =
	    300;
	...
	for (i = 0; i < data_cycle; i++) {
		...
		for (j = 0; j < node_num; j++) {
			node_data =
			    (raw_data[j * 4] | (raw_data[j * 4 + 1] << 8) |
			     (raw_data[j * 4 + 2] << 16) | (raw_data[j * 4 + 3]
							    << 24));
	...
	for (i = 0; i < syna_testdata->RX_NUM; i++) {
		...
		if (abs(max_store[i]) > judge_threshold) {
			TPD_INFO
			    ("Check RT59 for abs noise, data[%d] = %d(%d)\n", i,
			     max_store[i], judge_threshold);
			if (!error_count)
				seq_printf(s,
					   "Check RT59 for abs noise, data[%d] = %d(%d)\n",
...
static int synaptics_auto_test_rt63(struct seq_file *s,
				    struct chip_data_s3706 *chip_info,
				    struct syna_testdata *syna_testdata,
				    uint8_t * data_buf)
{
	...
	for (i = 0; i < syna_testdata->RX_NUM; i++) {
		node_data =
		    (raw_data[i * 4] | (raw_data[i * 4 + 1] << 8) |
		     (raw_data[i * 4 + 2] << 16) | (raw_data[i * 4 + 3] << 24));
		if (syna_testdata->fd >= 0) {
				...
				struct syna_testdata *syna_testdata)
{
	...
	if (!syna_testdata->fingerprint_underscreen_support) {
		ret =
		    synaptics_auto_test_rt155(s, chip_info, syna_testdata,
					      data_buf);
		if (ret > 0) {
			TPD_INFO
			    ("synaptics_auto_test_rt155 failed! ret is %d\n",
			     ret);
			error_count++;
			goto END;
		}
	...
	if (syna_testdata->fd_support) {
		if (chip_info->rt155_fdreplace_rt59_support) {
			ret =
			    synaptics_auto_test_rt155_fd(s, chip_info,
							 syna_testdata,
							 data_buf);
		} else {
			ret =
			    synaptics_auto_test_rt59(s, chip_info,
						     syna_testdata, data_buf);
		}
		if (ret > 0) {
			TPD_INFO
			    ("synaptics_auto_test_rt59/rt155 failed! ret is %d\n",
			     ret);
			error_count++;
			goto END;
		}
		...
		ret =
		    synaptics_auto_test_rt63(s, chip_info, syna_testdata,
					     data_buf);
		if (ret > 0) {
...
static int fwu_read_flash_status(struct chip_data_s3706 *chip_info)
{
	...
	retval = touch_i2c_read_block(chip_info->client, chip_info->fwu->f34_fd.data_base_addr + chip_info->fwu->off.flash_status, sizeof(status), &status);	//get ic state , idle or other
	if (retval < 0) {
	...
	retval = touch_i2c_read_block(chip_info->client, chip_info->fwu->f34_fd.data_base_addr + chip_info->fwu->off.flash_cmd, sizeof(command), &command);	//read recent command
	if (retval < 0) {
...
static int fwu_parse_image_info(struct chip_data_s3706 *chip_info)
{
	...
	if (chip_info->fwu->bl_version == BL_V7
		...
		fwu_parse_partition_table(chip_info, chip_info->fwu->img.fl_config.data, &chip_info->fwu->img.blkcount, &chip_info->fwu->img.phyaddr);	//firmware img in emmc
...
static int fwu_enter_flash_prog(struct chip_data_s3706 *chip_info)
{
	...
	retval = touch_i2c_write_block(chip_info->client, chip_info->fwu->rmi4_data->f01_ctrl_base_addr, sizeof(f01_device_control.data), f01_device_control.data);	//set no sleep mode
	if (retval < 0) {
				    ...
				    struct monitor_data *mon_data)
{
	...
	struct health_info *health_local =
	    &(chip_info->health_zone.health_info);
	...
	if (data_length < 1 || data_length > 255) {
		...
		if (health_info->baseline_err != 0
		    && health_local->baseline_err !=
		    health_info->baseline_err) {
		...
		if (health_info->reset_reason != 0) {
			...
			TPD_INFO
			    ("chip_info->reg_info.F51_CUSTOM_CTRL13= %x  ret=%x\n",
			     chip_info->reg_info.F51_CUSTOM_CTRL_BASE + 0x0C,
			     ret | 0x80);
		}
...
static void synaptics_gesture_rate(struct seq_file *s, u16 * coord_arg,
				   void *chip_data)
{
	int ret = 0;
	int loop = 0;
	...
	TPD_DETAIL("coord[0]:%u %u %u %u %u\n", coord_arg[0], coord_arg[1],
		   coord_arg[3], coord_arg[4], coord_arg[5]);
	ret = touch_i2c_write_byte(chip_info->client, 0xff, 0x00);	/* page 4 */
}
						...
						*fp_tpinfo)
{
	...
	touch_i2c_write_byte(chip_info->client, 0xff, 0x4);
	touch_i2c_read_block(chip_info->client,
			     chip_info->reg_info.F51_CUSTOM_DATA_BASE, 8,
			     &(buf[0]));
			      ...
			      const struct i2c_device_id *id)
{
	...
	chip_info->rt155_fdreplace_rt59_support =
	    of_property_read_bool(ts->dev->of_node,
